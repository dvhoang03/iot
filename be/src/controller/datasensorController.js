...
let search = (req, res) => {
    ...
    const vl = req.query.value;

    if (field === "timestamp") {
        const value = vl.replace("T", " ").concat("");
        console.log(field, vl)
        const query = `SELECT * FROM datasensor WHERE ${field} LIKE "%${value}%"  ORDER BY timestamp ASC LIMIT ? OFFSET ?`;
        ...
        db.query(query, values, (err, results) => {
            ...
            console.log(results)
            const countQuery = `SELECT COUNT(*) AS total FROM datasensor where ${field} LIKE "%${value}%" `;
        ...
        // Truy vấn dữ liệu từ cơ sở dữ liệu với phân trang
        const query = `SELECT * FROM datasensor WHERE ${field} = ${value}  ORDER BY timestamp ASC LIMIT ? OFFSET ?`;
        const values = [pageSize, offset];
        //query
        db.query(query, values, (err, results) => {
            if (err) {
            ...
            console.log(results)
            const countQuery = `SELECT COUNT(*) AS total FROM datasensor where ${field} = ${value} `;
...
let sort = (req, res) => {
    ...
    console.log(field, vl, vlsort);
    // Truy vấn dữ liệu từ cơ sở dữ liệu với phân trang và sắp xếp
    const query = `SELECT * FROM datasensor ORDER BY ${field} ${vlsort} LIMIT ? OFFSET ?`;
    const values = [pageSize, offset];
    // Thực thi truy vấn
    db.query(query, values, (err, results) => {
        if (err) {
...
module.exports = { getdata, search, sort };
